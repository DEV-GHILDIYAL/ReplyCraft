...
export default function SentimentPage() {
  ...
  const historical = hasNoBusiness ? [] : (data?.historical ?? []);
  const platformComparison = hasNoBusiness ? [] : (data?.platformComparison ?? []);
  const keywords = hasNoBusiness ? [] : (data?.keywords ?? []);
